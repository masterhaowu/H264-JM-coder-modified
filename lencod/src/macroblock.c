...
{
    printf("luma residual coding sp\n");
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  DecodedPictureBuffer *p_Dpb = p_Vid->p_Dpb_layer[p_Vid->dpb_layer_id];
                          ...
                          )
{
    ...
    if (currMB->best_mode == 1) {
        //printf("write motion vector 8x8\n");
    }
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  for (j = j0; j < j1; j += step_v)
  {
    ...
    for (i = i0; i < i1; i += step_h)
    {
      ...
      if (currMB->write_mb)
        ...
        //printf("best mode now is %d\n",currMB->best_mode);
        
        if (currMB->best_mode == 1) {
            
            //printf("check mv x: %d\n",cur_mv[0].mv_x);
            
            //printf("write cur mv x: %d\n",cur_mv->mv_x);
            //printf("write cur mv y: %d\n",cur_mv->mv_y);
...
{
    if (currMB->best_mode == 1) {
        //printf("p slice writing motion info\n");
    }
  int   no_bits         = 0;
  //=== If multiple ref. frames, write reference frame for the MB ===
  if (currMB->mb_type != I4MB  && currMB->mb_type != I16MB && currMB->mb_type != I8MB  && currMB->mb_type != 0)
