...
{
    //printf("setup fast full search for an macroblock/n");
    //currMB->fast_inter_decision_hw = 0;
  ...
  for   (y = currMB->opix_y; y < currMB->opix_y + MB_BLOCK_SIZE; y++)
  {
    ...
    srcptr += MB_BLOCK_SIZE;
      //printf("checking if curimg is changing: %d \n", p_Vid->pCurImg[0][0]);
      
      //p_Inp->IntraDisableInterOnly = 1;
      //p_Inp->DisableIntra16x16 = 1;
      //p_Inp->DisableIntra4x4 = 1;
    
      
      if (frame_changing_check_hw != p_Vid->frame_no) {
          
          
          
          printf("this is a new frame!\n");
          ...
          /*
          for (int i_hw = 0; i_hw < MB_BLOCK_SIZE; i_hw++) {
              for (int j_hw = 0; j_hw < MB_BLOCK_SIZE; j_hw++) {
                  //pre_frame_block_info_hw[i_hw][j_hw] = p_Vid->pCurImg[currMB->opix_y + i_hw][currMB->pix_x + j_hw];
                  SAD_hw = SAD_hw + abs((int)p_Vid->pCurImg[currMB->opix_c_y + i_hw][currMB->pix_x + j_hw] - (int)pre_frame_block_info_hw[i_hw][j_hw]);
              }
          }
          ...
          for (int i_hw = 0; i_hw < p_Vid->height; i_hw++) {
              for (int j_hw = 0; j_hw < p_Vid->width; j_hw++) {
                  pre_frame_block_info_hw[i_hw][j_hw] = pre_frame_info_hw[i_hw][j_hw];
              }
          }
          ...
          for (int i_hw = 0; i_hw < p_Vid->height; i_hw++) {
              for (int j_hw = 0; j_hw < p_Vid->width; j_hw++) {
                  SAFD_hw = SAFD_hw + abs((int)p_Vid->pCurImg[i_hw][j_hw] - (int)pre_frame_info_hw[i_hw][j_hw]);
              }
          }
          
          SAFD_hw = SAFD_hw/p_Vid->width/p_Vid->height;
          printf("the value of SAFD is: %f\n", SAFD_hw);
          //SAD_hw = SAD_hw/MB_BLOCK_SIZE/MB_BLOCK_SIZE;
          //printf("the value of SAD is: %f\n", SAD_hw);
          
          
          
          
          //printf("and the height of the frame is: %d\n", p_Vid->height);
          for (int i_hw = 0; i_hw < p_Vid->height; i_hw++) {
              for (int j_hw = 0; j_hw < p_Vid->width; j_hw++) {
                  pre_frame_info_hw[i_hw][j_hw] = p_Vid->pCurImg[i_hw][j_hw];
              }
          }
      }
      ...
      for (int i_hw = 0; i_hw < MB_BLOCK_SIZE; i_hw++) {
          for (int j_hw = 0; j_hw < MB_BLOCK_SIZE; j_hw++) {
              //pre_frame_block_info_hw[i_hw][j_hw] = p_Vid->pCurImg[currMB->opix_y + i_hw][currMB->pix_x + j_hw];
              SAD_hw = SAD_hw + abs((int)p_Vid->pCurImg[currMB->opix_c_y + i_hw][currMB->pix_x + j_hw] - (int)pre_frame_block_info_hw[currMB->opix_c_y + i_hw][currMB->pix_x + j_hw]);
          }
      }
      ...
      if (SAD_hw * w_value_hw < SAFD_hw) {
          currMB->fast_inter_decision_hw = 1;
          //printf("skipping \n");
      }
      
      currMB->MB_SAD_hw = SAD_hw;
      
      //frame_changing_check_hw = p_Vid->pCurImg[0][0];
      frame_changing_check_hw = p_Vid->frame_no;
      
      
      //struct macroblock_enc * MB_up_hw = currMB->mb_up;
      //distblk temp_cost_hw1 = MB_up_hw->min_rdcost;
      //struct macroblock_enc * MB_up_hw2 = currMB->mb_left;
      //distblk temp_cost_hw2 = MB_up_hw2->min_rdcost;
      
      
            
      /*
       if (p_Vid->image_MAFD_done_hw == 0) {
          printf("first time this frame\n");
          p_Vid->image_MAFD_done_hw = 1;
      ...
      //memcpy(srcptr_temp_hw, &p_Vid->pCurImg[y][currMB->pix_x], MB_BLOCK_SIZE * sizeof(imgpel));
      //srcptr_temp_hw += MB_BLOCK_SIZE;
  }
...
{
    //printf("fast full search motion estimation\n");
