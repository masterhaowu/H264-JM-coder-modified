...
void encode_one_macroblock_high (Macroblock *currMB)
{
  ...
  if (!intra)
  {
    ...
    {    
      ...
      currMB->valid_4x4 = FALSE;
        ...
        //change this part for fast inter decision Hao Wu
      if (p_Inp->Transform8x8Mode != 2 && currMB->fast_inter_decision_hw == 0)
       // if (p_Inp->Transform8x8Mode != 2)
      {
          //printf("this is enabled?\n");
  ...
  for (currMB->c_ipred_mode = chroma_pred_mode_range[0]; currMB->c_ipred_mode<=chroma_pred_mode_range[1]; currMB->c_ipred_mode++)
  {
    ...
    if ( (p_Vid->yuv_format != YUV400) &&
      ...
      Boolean SKIP_intra_mode_hw = 1;
    
      distblk threshold_rd_cost_hw = 0;
      distblk min_cost_1_hw = 0;
      distblk min_cost_2_hw = 0;
      distblk min_cost_3_hw = 0;
      
      if (currMB->pix_x > 20 && currMB->pix_y>20) {
          min_cost_1_hw = currMB->mb_up->min_rdcost;
          min_cost_2_hw = currMB->mb_left->min_rdcost;
          min_cost_3_hw = currMB->mb_up->mb_left->min_rdcost;
          threshold_rd_cost_hw = min_cost_1_hw;
          if (threshold_rd_cost_hw > min_cost_2_hw) {
              threshold_rd_cost_hw = min_cost_2_hw;
          }
          if (threshold_rd_cost_hw > min_cost_3_hw) {
              threshold_rd_cost_hw = min_cost_3_hw;
          }
          
      }
      //printf("checking SAD: %f \n",currMB->MB_SAD_hw);
      if (threshold_rd_cost_hw <= currMB->MB_SAD_hw) {
          SKIP_intra_mode_hw = 0;
      }
    ...
    for (index=0; index < max_index; index++)
    {
      ...
      if (enc_mb.valid[mode])
      {
        ...
        else
        {
         ...
         // if (index < 6) {
          ...
          //change this part for fast intra decision Hao Wu
     ...
     // printf(" best %d %7.2f\n", currMB->best_mode, (double) currMB->min_rdcost);
      ...
      if (currMB->best_mode == 1) {
          //printf(" best %d %7.2f\n", currMB->best_mode, (double) currMB->min_rdcost);
          //int printing_vector_hw = currMB->p_Slice->all_mv[0][0][0][0][0].mv_x;
          //printf("checking vector x: %d \n", printing_vector_hw);
      }
      //check_total_mb_hw++;
      //printf(" best %d %7.2f\n", currMB->best_mode, (double) currMB->min_rdcost);
